...
##########Submitted Code #########
* I deleted the following files and folders according to the instruction (node_modules,.git,database.db)
* Also make a video of explaining the functionalities also follow this "```npm install```, ```npm run build-db```, and ```npm run start```"  this specific codes to makes sure its runing in my Windows Os. 
* Jus to inform i using "type db_schema.sql | sqlite3 database.db" this code to run my database_schema.sql since i using windows OS
* Also i never delete anyhting from the template provided in the coursera instead i add my codes in the provided template. 
